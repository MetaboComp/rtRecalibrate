...
lama_rt_correction <- function(rt,
    ...
    if (method == "GAM") {
        fit <- makeGAM(
            RTDF = rtMap, bs = bs, ssqRatio = ssqRatio,
            weights = weights
        )
    } else {
        fit <- makeLoess(
            RTDF = rtMap, span = span, ssqRatio = ssqRatio,
            weights = weights
        )
    }
    ...
    ## Making plot of fit
    if (plot) {
        plot((reference - observed) ~ observed,
            data = rtMap,
...
makeGAM <- function(RTDF, bs, ssqRatio, weights = weights) {
    gam <- gam(reference ~ s(observed, bs = bs),
        weights = weights,
        data = RTDF
    )
    pred <- predict(gam)
...
makeLoess <- function(RTDF, span, ssqRatio, weights = weights) {
    loess <- loess(reference ~ observed,
        weights = weights, span = span,
        data = RTDF
    )
    pred <- predict(loess)
    SSq <- resid(loess)^2
