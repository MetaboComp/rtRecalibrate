                           ...
                           maxLM=200)
{
  ...
  if(missing(Prefilterintensity)){
    stop("The prefilter intensity (xcms) or equivalent (in other peak picking
         software) needs to be supplied in order to correctly determine which
         peaks are eligble to become landmarks.")
  }
  
  if(ncol(PT_NoFill) < minLM){
    stop("The number of features in the peak table are not enough to satisfy the
          minLM criterium. Please lower the minLM or look over your peak table.")
  }
  ...
  minIntensity <- seq(from=Prefilterintensity,
  ...
  foundLMs <- list()
  for (i in 1:nrow(Des)){
    foundLMs[[i]] <- findLandmarks(dat = PT_NoFill,
                                   mzrtdf = mzrtdf,
                                   mzdiff = Des$mzdiff[i],
                                   rtdiff = Des$rtdiff[i],
  ...
  Settings <- list(rank1settings,
                   ...
                   rank9settings,
                   rank10settings)
  Overview <- rbind(as.matrix(rank1settings),
  ...
  colnames(Overview) <- c("minIntensity",
                          "allowedMissingness",
                          "mzdiff",
                          "rtdiff",
                          "coverage")
...
calcCoverage <- function(rts, runTime){
  ...
  # interval <- c(vapply(rts, FUN=makeInterval, FUN.VALUE=numeric(1)))
