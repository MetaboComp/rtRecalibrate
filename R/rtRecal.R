...
rtRecal <- function(file,
  ...
  if(!file.exists(file)){
    stop(paste0("The file doesn't exist. Please",
                " choose the path to a real file."))
  }
  ...
  if(nrow(lamas) < 5 || ncol(lamas) > 2 || !is.data.frame(lamas)){
    stop("Wrong format for lamas input. Should be a data frame with 2 columns \"mz\" and \"rt\" and with at least 5 lamas")
  }
  ...
  peaks <- lama_peak_detection(x = ms,
                                lamas = lamas,
                                ppm = ppm_roi,
                                expandRt = expandRt,
                                peakwidth = peakwidth)
